...
# >>> python train_ner_flair.py -c /path/to/corpus/ -m /path/to/model/folder/ -o options_ner_flair [--continue_training] [--tensorboard] -e fwd-lm.pt bwd-lm.pt
...
if __name__ == "__main__":
    ...
    if args.continue_training:
        checkpoint = tagger.load_checkpoint(args.model_path + 'checkpoint.pt')
        trainer = ModelTrainer.load_from_checkpoint(checkpoint, corpus)
    else:
        trainer = ModelTrainer(tagger, corpus, use_tensorboard=args.tensorboard)
    # training
    trainer.train(args.model_path, **options['training'])
